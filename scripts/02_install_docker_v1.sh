...
if command -v docker &>/dev/null; then
  ...
  # === Wait for dpkg lock if held ===
  echo -e "\e[91m🔍 Checking for dpkg lock...\e[0m"
  ...
  while fuser "$LOCK_FILE" &>/dev/null; do
    if [ "$WAITED" -ge "$MAX_WAIT" ]; then
      echo -e "\e[91m❌ dpkg lock still held after $MAX_WAIT seconds. Aborting.\e[0m"
    ...
    echo -e "\e[91m⏳ dpkg is locked. Waiting... ($WAITED/$MAX_WAIT seconds)\e[0m"
    sleep 5
    WAITED=$((WAITED + 5))
  done

  echo -e "\e[91m✅ dpkg lock released. Proceeding with Docker installation...\e[0m"
