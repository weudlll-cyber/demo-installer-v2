...
✅ DNS wait and retry for GitHub access ✅ apt/dpkg lock detection and recovery ✅ Bun and Docker installation ✅ Node repo cloning and dependency install ✅ Systemd service creation ✅ Public IP detection and peerlist setup ✅ Key backup, restart, stop, and health-check helpers ✅ One-time reboot with resume logic ✅ Smart skipping of already-installed components ✅ Bright red output for all user-facing messages

🚀 Quick Start
To install a Demos node in one step:

bash
curl -fsSL https://raw.githubusercontent.com/weudl/demos-node-installer/main/demos_node_setup_v1.sh | bash
To just install the helper programs:

bash
curl -fsSL https://raw.githubusercontent.com/weudl/demos-node-installer/main/scripts/install_helpers_v1.sh | bash
💡 The script will automatically reboot once to finalize system upgrades. After reboot, re-run the same command or execute the script locally if you saved it.
...
Marker-based logic: Each step writes a marker to /root/.demos_node_setup for safe re-entry.
...
bash
rm /root/.demos_node_setup/02_install_docker_v1.done
...
🧑‍💻 Maintainer
Built and maintained by Weudl Focused on privacy infrastructure, reproducible workflows, and community education.